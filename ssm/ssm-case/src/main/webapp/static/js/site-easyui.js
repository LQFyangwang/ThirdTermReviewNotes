 ...
 * @param prefix
 */
function openEditWinPrefix(listId, winId, formId, prefix) {
    var row = $("#" + listId).datagrid("getSelected");
    if (row) {
        var data = datagridRowHandler(row, prefix);
...
function getQueryParams(dataGridId, formId) {
    ...
    var params = $("#" + dataGridId).datagrid('options').queryParams; // page rows
...
function getPagerOptions(listId) {
    return $("#" + listId).datagrid("getPager").data("pagination").options;
}

function doSearch(listId, formId) {
    var options = getPagerOptions(listId);
    $("#" + listId).datagrid({
        url:options.url,
...
function save(url, formId, listId, winId) {
    saveOrEdit(url, formId, listId, winId);
}

function edit(url, formId, listId, winId) {
    saveOrEdit(url, formId, listId, winId);
}

function saveOrEdit(url, formId, listId, winId) {
...
function fillDataFromJson(data) {
    for (var key in data) {
        $("#" + key).text(data[key]);
    }
}